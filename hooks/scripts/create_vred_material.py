...
def render_material(material, render_name, render_path, render_geometry_path):
    """
    Render the Material.

    Create a render automation for the material.
      Create geometry
        - load in by file?
        - Default to creating basic geometry
    ...
    n = 1
    increment = 180.0
    final_rotation = rotation.z() + increment
    while rotation.z() < final_rotation:
        # Rotate the camera
        rotation.setZ(rotation.z() + increment)
...
# Check if we should render a scene with the material applied to it
if render_path:
    success, error = render_material(material, render_name, render_path, render_geometry_path)
    if not success:
        terminate_vred(error)
