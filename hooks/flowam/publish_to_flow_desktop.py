...
class DesktopFlowPublishPlugin(HookBaseClass):
    """
    Desktop publish plugin for Flow Asset Management.

    Sits on top of ``FlowPublishPlugin`` in the hook chain::

        hook: "{self}/publish_file.py:{self}/flowam/publish_to_flow.py:{self}/flowam/publish_to_flow_desktop.py"

    Rejects Maya files in ``accept`` (Maya dependencies are not tracked from
    the desktop context), adds desktop-specific revision validation, and
    publishes via either ``publish_generic_revision`` (existing asset) or
    ``create_generic_workfile`` (new asset).
    """

    def accept(self, settings, item):
        """
        Reject Maya files (.ma/.mb) - Maya dependencies are not tracked in
        Flow AM from the desktop context. Otherwise defer to the base accept.
        """
    ...
    def validate(self, settings, item):
        """
        Run the base project validation, then (if publishing to an existing
        asset) validate that the parent's revision corresponds to a generic
        asset.
        """
        if not super().validate(settings, item):
            return False

        # Validate revision asset type only if publishing to an existing asset
        revision_id = None
        if item.parent:
    ...
    def _publish_to_flow(self, item):
        """
        Delegate to ``_publish_revision`` or ``_create_new_asset`` depending
